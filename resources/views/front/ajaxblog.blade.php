    ...
    @foreach($blogs as $list)
                    <div class="col-lg-4 col-md-6 mt-1-9">
                        <article class="card card-style1">
                            <div class="card-img"><img alt="@if(isset($list->image_alt)){{$list->image_alt}}@endif" src="{{asset('blogs')}}/{{$list->image}}"  height="50">
                            </div>
                            <div class="card-body">
                                <!-- <span></span> -->
                                <div class="card-date">
                                ...
                                <h3 class="h5"><a href="{{route('front.blog',$list->url)}}">{{$list->heading}}</a></h3>
                                <p class="card-text">{!! html_entity_decode(wordLimitset($list->description,80)) !!}</p>
                                <a href="{{route('front.blog',$list->url)}}" class="butn small"><span>Read More</span></a>
                            </div>
                        </article>
                    </div>
                    @endforeach
