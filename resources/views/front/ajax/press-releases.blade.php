...
<ul class="list_blog grid lg:gap-6 gap-5 w-full">
    ...
    <li class="blog_item flex max-sm:flex-col sm:items-start gap-8 gap-y-5 w-full border border-line p-4 rounded-lg animate animate_top" style="--i: 1">
        <a href="{{ route('front.press', $list->url) }}" class="blog_thumb block">
            <img class="w-full overflow-hidden rounded-lg" src="{{ $list->image }}?updatedAt=1738998257194" loading="lazy" alt="image">
        </a>
        <div class="">
            ...
            <a href="{{ route('front.press', $list->url) }}" class="heading6 blog_title mt-1 hover:underline">{{ $list->heading }}</a>
            <p class="blog_desc mb-3">{!! html_entity_decode(wordLimitset($list->description, 80)) !!}</p>
            <a href="{{ route('front.press', $list->url) }}" class="text-button pb-0.5 border-b-2 border-primary duration-300 text-primary">Know more <i class="ph-bold ph-arrow-bend-up-right ml-2"></i></a>
        </div>
    </li>
    @endforeach
